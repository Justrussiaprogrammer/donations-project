...
- full_visible_text должен содержать весь видимый текст сверху вниз.
- visible_lines должен содержать видимые строки сверху вниз.

Правила суммы:
- Восклицательный знак после суммы — декоративный. Не включай его в amount_text.
- Если написано "200!", amount_text = "200", amount_number = 200, currency = null.
- Если написано "200 RUB!", amount_text = "200 RUB", amount_number = 200, currency = "RUB".
...
- Если слово плохо читается, запиши наиболее вероятный вариант и добавь warning.

Правила confidence:
- confidence — число от 0 до 1.
- Если сумма, donor и message читаются хорошо, confidence может быть высоким.
- Если есть сомнительные слова, confidence должен быть ниже 0.90.
- Если есть сомнение в donor, сумме или message, needs_review = true.

Верни JSON строго по этой схеме:
{
  "visible_lines": ["строка 1", "строка 2"],
  "donor": string или null,
  "amount_text": string или null,
  "amount_number": number или null,
  "currency": string или null,
  "message": string или null,
  "full_visible_text": string,
  "confidence": number,
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    for ev in events:
        ...
        if args.skip_vlm:
            ...
            raw_text, parsed, model_error = call_vlm_for_crop(
        ...
        visible_lines = parsed.get("visible_lines") or []
        ...
        row = {
            ...
            "detections_count": ev.detections_count,
            "best_detection_confidence": round(ev.best_confidence, 4),
            "best_detection_timestamp_sec": round(best_det.timestamp_sec, 3),
            "best_detection_time": seconds_to_timestamp(best_det.timestamp_sec),
            ...
            "best_crop_path": str(best_crop_path),
            "best_frame_path": str(best_frame_path),
            "parsed_ok": parsed_ok,
            "donor": parsed.get("donor", ""),
            "amount_text": parsed.get("amount_text", ""),
            "amount_number": parsed.get("amount_number", ""),
            "currency": parsed.get("currency", ""),
            "message": parsed.get("message", ""),
            "full_visible_text": parsed.get("full_visible_text", ""),
            "visible_lines_json": json_dumps_compact(visible_lines),
            "confidence": parsed.get("confidence", ""),
            "needs_review": parsed.get("needs_review", True),
            "warnings_json": json_dumps_compact(warnings),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--vlm-max-tokens", type=int, default=640)
